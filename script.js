...
import { getDatabase, ref, get, set, onValue, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-database.js";
...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!currentUser) {
            ...
            voteForm.innerHTML = `
                <div style="text-align: center; padding: 2rem 0;">
                    ...
                    <p style="margin-bottom: 2rem; color: var(--text-secondary);">You have successfully locked in your vote for <strong>${votedFor}</strong>.</p>
                    <a href="percentage.html" class="btn-primary" style="display: inline-block;">View Live Results</a>
                </div>
            `;
        } else {
            const radioButtons = voteForm.querySelectorAll('input[name="party"]');
            if (radioButtons.length > 0) {
                radioButtons.forEach(radio => {
                    radio.addEventListener('change', function () {
                        voteForm.querySelectorAll('.vote-option').forEach(opt => opt.classList.remove('selected'));
                        if (this.checked) {
                ...
                // FIX 3: Clone to remove stale listeners
                ...
                newVoteForm.addEventListener('submit', async (e) => {
                    ...
                    // FIX 4: Scope querySelector to newVoteForm
                    const selectedParty = newVoteForm.querySelector('input[name="party"]:checked');
                    ...
                    if (selectedParty && currentUser) {
                        ...
                        try {
                            ...
                            if (snapshot.exists()) {
                                ...
                                // FIX 3: Re-enable button on early return
    ...
    if (totalVotesCountEl) {
        ...
        onValue(totalsRef, (snapshot) => {
            ...
            parties.forEach(party => {
                ...
                const rawPercent = totalVotes === 0 ? 0 : (count / totalVotes) * 100;
    ...
    if (googleSignInBtn) {
        // FIX 2: Use innerHTML consistently
        ...
        googleSignInBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (err.code === 'auth/unauthorized-domain') {
                    authMsg.innerHTML = "<strong>Security Error:</strong> You are opening this file directly. Please use a local web server (like VS Code Live Server) or add this domain in Firebase Console.";
                } else if (err.code === 'auth/popup-closed-by-user') {
                    authMsg.innerText = "Sign-in was cancelled. Please try again.";
                } else {
